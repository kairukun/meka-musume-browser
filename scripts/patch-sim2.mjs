...
s = s.replaceAll("SIM_DEPLOY", "__DEPLOY__");
s = s.replaceAll("SIM_MAP[y][x]", "battle.map[y][x]");
s = s.replaceAll("tileImage(x, y)", "tileImage(battle.map, x, y)");
// In helpers, battle is the param name for clickTile — __DEPLOY__ -> battle.deploy
// In JSX setBattle callbacks using b — need b.deploy
s = s.replace(
  `setBattle((b) => ({
                          ...b,
                          selected: u.id,
                          mode: "place",
                          moveTiles: [...__DEPLOY__],
...
s = s.replaceAll("__DEPLOY__", "battle.deploy");
// JSX uses battle.deploy which is correct for render; clickTile uses battle param — good
...
if (!s.includes(oldChip)) {
  console.error("chip block missing");
  process.exit(1);
}
s = s.replace(oldChip, newChip);

const close = "        </div>\n      </div>\n    </div>\n  );\n}";
const ci = s.lastIndexOf(close);
if (ci < 0) {
